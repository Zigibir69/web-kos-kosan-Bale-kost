...
// Inisialisasi session penyembunyian
if (!isset($_SESSION['pesanan_disembunyikan'])) {
    $_SESSION['pesanan_disembunyikan'] = [];
}

// Sembunyikan pesanan
if (isset($_POST['hapus_pesanan'])) {
    $id_pesanan = $_POST['id_pesanan'];
    if (!in_array($id_pesanan, $_SESSION['pesanan_disembunyikan'])) {
        $_SESSION['pesanan_disembunyikan'][] = $id_pesanan;
    }
    echo "<script>alert('Pesanan berhasil disembunyikan.'); window.location='status_pesanan.php';</script>";
    exit;
}
...
if (isset($_POST['batal_pesanan'])) {
    $id_pesanan = $_POST['id_pesanan'];
    $stmt = $koneksi->prepare("UPDATE tbl_pesanan SET status = 'menunggu', tanggal_masuk = NULL, tanggal_berakhir = NULL WHERE id_pesanan = ? AND id_user = ?");
    $stmt->bind_param("ii", $id_pesanan, $id_user);
    $stmt->execute();
    echo "<script>alert('Pesanan berhasil dibatalkan dan dikembalikan ke status menunggu.'); window.location='status_pesanan.php';</script>";
    exit;
}
...
<head>
    ...
    <style>
        ...
        .badge-status { font-size: 0.85rem; padding: 0.4em 0.65em; }
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover align-middle">
                        ...
                        <?php $no = 1; while ($row = $result->fetch_assoc()): ?>
                            <?php 
                                if (in_array($row['id_pesanan'], $_SESSION['pesanan_disembunyikan'])) continue;

                                $tanggal_masuk = ($row['status'] === 'disetujui' && $row['tanggal_masuk']) 
                                    ? date('d-m-Y', strtotime($row['tanggal_masuk'])) 
                                    : '-';

                                $tanggal_berakhir = ($row['status'] === 'disetujui' && $row['tanggal_berakhir']) 
                            ...
                            <tr class="text-center">
                                ...
                                <td>
                                    ...
                                    <?php elseif ($row['status'] == 'disetujui'): ?>
                                        <a href="perpanjang_kontrak.php?id_pesanan=<?= $row['id_pesanan']; ?>" class="btn btn-sm btn-primary">Perpanjang</a>
                                        <form method="post" class="d-inline" onsubmit="return confirm('Batalkan persetujuan?')">
                                            <input type="hidden" name="id_pesanan" value="<?= $row['id_pesanan']; ?>">
                                            <button type="submit" name="batal_pesanan" class="btn btn-sm btn-warning">Batal</button>
                                        </form>
                                        <form method="post" class="d-inline" onsubmit="return confirm('Hapus pesanan ini?')">
                                            <input type="hidden" name="id_pesanan" value="<?= $row['id_pesanan']; ?>">
                                            <button type="submit" name="hapus_pesanan" class="btn btn-sm btn-danger">Hapus</button>
